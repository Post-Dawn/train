...
def evaluate_performance(all_target, predicted, toplot=True, verbose=False):
    ...
    if verbose == True:
        ...
        print ('At threshold=' + str(np.round(thresholds[maxind], 3)) + ', TPR=' + str(np.round(tpr[maxind], 2)) + ', ' + str(
            int(np.round(tpr[maxind] * event_rate * all_target.shape[0]))) + ' out of ' + str(
            int(np.round(event_rate * all_target.shape[0]))))
        print ('At threshold=' + str(np.round(thresholds[maxind], 3)) + ', FPR=' + str(np.round(fpr[maxind], 2)) + ', ' + str(
...
def evaluate_performance_sim(all_target, predicted):
    ...
    # print ('recall= ' + str(round(recall_, 2)) +
    #        ', precision= '+ str(round(precision_, 2))+
